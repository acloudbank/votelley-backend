...
Vote::Vote(const Row &r, const ssize_t indexOffset) noexcept
{
    if(indexOffset < 0)
    {
        ...
        if(!r["vote"].isNull())
        {
            vote_=std::make_shared<int32_t>(r["vote"].as<int32_t>());
        }
    }
    else
    {
        ...
        if(!r[index].isNull())
        {
            vote_=std::make_shared<int32_t>(r[index].as<int32_t>());
        }
    }
...
Vote::Vote(const Json::Value &pJson, const std::vector<std::string> &pMasqueradingVector) noexcept(false)
{
    ...
    if(!pMasqueradingVector[2].empty() && pJson.isMember(pMasqueradingVector[2]))
    {
        ...
        {
            vote_=std::make_shared<int32_t>((int32_t)pJson[pMasqueradingVector[2]].asInt64());
        }
    }
}
...
Vote::Vote(const Json::Value &pJson) noexcept(false)
{
    ...
    if(pJson.isMember("vote"))
    {
        ...
        {
            vote_=std::make_shared<int32_t>((int32_t)pJson["vote"].asInt64());
        }
    }
}
...
const int32_t &Vote::getValueOfVote() const noexcept
{
    static const int32_t defaultValue = int32_t();
    if(vote_)
        return *vote_;
...
const std::shared_ptr<int32_t> &Vote::getVote() const noexcept
{
    return vote_;
}
void Vote::setVote(const int32_t &pVote) noexcept
{
    vote_ = std::make_shared<int32_t>(pVote);
    dirtyFlag_[2] = true;
}
